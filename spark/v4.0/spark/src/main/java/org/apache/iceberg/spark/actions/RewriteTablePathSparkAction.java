...
public class RewriteTablePathSparkAction extends BaseSparkAction<RewriteTablePath>
  ...
  private Set<Pair<String, String>> rewriteVersionFile(
    ...
    String stagingPath = RewriteTablePathUtil.stagingPath(versionFilePath, stagingDir, sourcePrefix);
  ...
  private Set<Pair<String, String>> statsFileCopyPlan(
    ...
    for (int i = 0; i < beforeStats.size(); i++) {
      ...
      result.add(
          Pair.of(
              RewriteTablePathUtil.stagingPath(before.path(), stagingDir, sourcePrefix),
              after.path()));
    }
    return result;
  }
  ...
  private RewriteResult<ManifestFile> rewriteManifestList(
    ...
    String outputPath = RewriteTablePathUtil.stagingPath(path, stagingDir, sourcePrefix);
  ...
  private static RewriteResult<DataFile> writeDataManifest(
    ...
    try {
      String stagingPath =
          RewriteTablePathUtil.stagingPath(manifestFile.path(), stagingLocation, sourcePrefix);
    ...
    return deleteFile -> {
      ...
      String newPath =
          RewriteTablePathUtil.stagingPath(
              deleteFile.location(), stagingLocationArg, sourcePrefixArg);
